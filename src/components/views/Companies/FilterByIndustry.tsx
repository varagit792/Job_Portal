...
import { getCompanyTypeList, getIndustryList } from '../../utils/utils';
import { setCompanyType, setNavigateFilterOption, setCheckItems, setCompanyTypeIds, setIndustry, setIndustryIds } from '../../../store/reducers/companies/getAllCompanies';
import { ChevronUpIcon } from '@heroicons/react/20/solid';
import { BiSearch } from 'react-icons/bi';

export const IndustryFilter = ({ handleIndustryCheckbox, setIsOpen }: any) => {
    const dispatch = useAppDispatch();
    const { industry } = useAppSelector((state) => state.getAllCompanies);
    useEffect(() => {
        (async () => {
            const industryList = await getIndustryList();
            if (Object.keys(industryList)?.length) {
                dispatch(setIndustry(industryList));
            }
        })();
    }, []);
    const handleViewAll = () => {
        setIsOpen(true)
        dispatch(setNavigateFilterOption("Industry"));
    }
    return (
                ...
                {({
                    open
                        ...
                        <Disclosure.Panel className="mt-5">
                            {industry?.slice(0, 5)?.map((item: any) => <div className="text-[#475569] mb-2 flex justify-start items-center">
                                <input type="checkbox" defaultChecked={false} checked={item?.isChecked} onChange={() => handleIndustryCheckbox(item)} />
...
const FiltersIndustry = () => {
    ...
    const handleIndustryCheckbox = (data: any) => {
        dispatch(setCheckItems({
            industry: checkItems?.industry?.map((item: any) =>
                item?.id === data?.id ? { ...item, isChecked: !item.isChecked } : item
            )
        }));
        if (data?.isChecked === undefined || data?.isChecked === false) {
            dispatch(setIndustryIds(data?.id));
        } else {
            dispatch(setIndustryIds({ filter: data?.id }));
        }
    };
    ...
    const filteredItems = checkItems?.industry?.filter((item: any) =>
...
export default FiltersIndustry;