...
const AllJobs = () => {
    ...
    const { success: successFilter, allJobs: filterAllJobs, loading: loadingFilter } = useAppSelector((state) => state.getFilterJobs);
    ...
    useEffect(() => {
        dispatch(getFilterJobs({ page: page, data: filtersData }));
    }, [filtersData]);
    ...
    useEffect(() => {
        ...
        if (successFilter) {
            if (filterAllJobs?.length == 0) {
                setToggleDispach(false);
            }
            setJobCard(filterAllJobs);
    ...
    const expCalculate = (y: number) => {
        let i = 0;
        let filterExperienceYears: any;
        for (i; i <= y; i++) {
            filterExperienceYears = experienceYears.filter((item: any) => {
        ...
        setFiltersData((preValue: any) => {
            return {
                ...preValue,
                expYear: filterExperienceYears?.map((item: any) => item?.title)
            }
        });
    };
    const handleTotalExpYearChange = async (event: React.FormEvent<HTMLInputElement> | any) => {
        setTotalExpYear(event.target.value);
        expCalculate(event.target.value);
        setPage(1);
    };

    const handleSalaryChange = (event: React.FormEvent<HTMLInputElement> | any) => {
        // RangeCalculate(event.target.value)
                                        ...
                                        <Disclosure.Panel className="mt-12">
                                            <div className="relative mb-3">
                                                ...
                                                <input className="w-full h-1 rounded-lg cursor-pointer overflow-hidden appearance-none bg-[#C7D2FE]"
                                                    ...
                                                    max="50"
                                                    value={totalExpYear}
                                                    onChange={handleTotalExpYearChange}
                                                />
                                            </div>
                                            <div className="flex justify-between items-center text-[#64748B] text-xs">
                                                <span>0 Yrs</span>
                                                <span>{experienceYears?.[experienceYears?.length - 1]?.title}</span>
                                            </div>
                                        </Disclosure.Panel>
                                    </>
                                )}
